    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        BTN_signup_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                adminDTO.setHOTENADMIN(hoTen);
                adminDTO.setTENDN(tenDN);
                adminDTO.setEMAIL(eMail);
                adminDTO.setSDT(sDT);
                adminDTO.setMATKHAU(matKhau);
                adminDTO.setGIOITINH(gioiTinh);
                adminDTO.setNGAYSINH(ngaySinh);

                //nếu nhân viên đầu tiên đăng ký sẽ có quyền quản lý
                if(!adminDAO.KtraTonTaiNV()){
                    quyenDAO.ThemQuyen("Quản lý");
                    quyenDAO.ThemQuyen("Nhân viên");
                    adminDTO.setMAQUYEN(1);
                }else {
                    adminDTO.setMAQUYEN(2);
                }

                //Thêm nv dựa theo obj nhanvienDTO
                long ktra = adminDAO.ThemNhanVien(adminDTO);
                if(ktra != 0){
